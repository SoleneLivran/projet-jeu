...
| En tant que | Je veux | Afin de (si besoin/nécessaire) | Version
|--|--|--|--|
| utilisateur | pouvoir quitter une partie en cours  | afin de sortir du mode jeu | MVP
| utilisateur | pouvoir recommencer une partie si le jeu est terminé | afin de retenter l'aventure | MVP
| utilisateur | pouvoir entrer dans une interface de jeu immersive | afin d'être plongé dans un mode de jeu où la barre de navigation disparait et laisse place à un bouton "quitter" | MVP
| utilisateur | pouvoir noter un jeu une fois terminé | afin de générer un rating des jeux | MVP
| utilisateur | pouvoir faire des points de sauvegarde de ma partie en cours | afin de la terminer plus tard | BONUS
| utilisateur | pouvoir profier d'une ambiance sonore | afin d'améliorer mon expérience de jeu | BONUS

## Create interface

| En tant que | Je veux | Afin de (si besoin/nécessaire) | Version
|--|--|--|--|
| utilisateur | pouvoir choisir le titre de mon scénario | afin qu'il apparaisse dans la liste des jeux | MVP
| utilisateur | pouvoir écrire un synopsys de mon scénario | afin de décrire l'univers de mon jeu | MVP
| utilisateur | pouvoir choisir un niveau de difficulté à titre indicatif | afin d'indiquer aux autres utilisateurs un niveau de difficulté  | MVP
| utilisateur | pouvoir sauvegarder un scénario en cours de création | afin de poursuivre l'édition de son scénario plus tard | MVP
| utilisateur | pouvoir modifier un jeu que j'ai créé | afin de modifier le scénario  | MVP
| utilisateur | pouvoir publier un jeu que j'ai créé | afin qu'il soit jouable et public | MVP
| utilisateur | pouvoir attribuer un genre à mon scénario parmi une liste  | afin de pouvoir le retrouver à l'aide des filtres  | MVP
| utilisateur | pouvoir choisir un lieu où se passe la scène en cours | afin de donner une ambiance spatiotemporelle | MVP
| utilisateur | pouvoir choisir les évènements qui s'y déroule | afin de dynamiser le scénario et créer des actions en conséquences | MVP
| utilisateur | pouvoir choisir les actions qui y ont lieu | afin de créer des choix ou des contraintes  | MVP
| utilisateur | pouvoir créer un module | afin de pouvoir utiliser ses propres modules dans l'éditeur d'histoire | BONUS
| utilisateur | pouvoir personnaliser un module | afin de pouvoir modifier la description, l'avatar et le titre d'un module déjà existant | BONUS
| utilisateur | pouvoir uploader un fichier image d'un module créé | afin de le personnaliser encore plus | BONUS
| utilisateur | pouvoir attribuer une ambiance sonore à mes scènes | afin de personnaliser mon histoire | BONUS

## Back Office

| En tant que | Je veux | Afin de (si besoin/nécessaire) | Version
|--|--|--|--|
| Administrateur | pouvoir me connecter à l'interface d'administration |  | MVP
| Administrateur | visualiser tous les utilisateurs | Accéder la liste utilisateur détaillée selon la BDD | MVP
| Administrateur | ajouter un utilisateur |  | MVP
| Administrateur | modifier un utilisateur |  | MVP
| Administrateur | supprimer un utilisateur |  | MVP
| Administrateur | visualiser tous les jeux  | (avec leurs détails : synopsys, genre, difficulté, title, avatar, publié ou en création) et aussi permettre de voir afin de voir leur date de modification / mise à jour et nettoyer la base de donnée | MVP
| Administrateur | ajouter un jeu |  | MVP
| Administrateur | modifier un jeu | modifier uniquement son descriptif (on ne retouche pas l'histoire) | MVP
| Administrateur | supprimer un jeu |  | MVP
